...
//!
//! Same framing as `clhorded`'s socket: each frame is length-delimited
//! (4-byte big-endian length + JSON payload). Requests are tagged via
//! `#[serde(tag = "type")]` so the server can dispatch on the variant.
//!
//! The minimal request set is `Status` / `Cancel` / `Retry` plus a
//! `Ping` liveness check — that's exactly what the plan calls for in
//! Phase 3. Future variants stay backwards-compatible because every
//! optional field carries `#[serde(default)]`.
...
mod tests {
    ...
    fn workflow_summary_round_trip_minimal() {
        // Only required fields set — make sure the back-compat defaults
        // hold after a serialize/deserialize round-trip.
    ...
    fn workflow_summary_back_compat_missing_optionals() {
        // A future scheduler might add fields. Older clients should still
        // parse a minimal payload.
